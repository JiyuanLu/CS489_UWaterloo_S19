...
class Layer():
    ...
    def __init__(self, n_nodes=0, act=None):
        '''
    ...
    def Save(self, fp):
        ...
        np.save(fp, self.h)
        np.save(fp, self.z)
        np.save(fp, self.mask)
        np.save(fp, self.b)

    def Load(self, fp):
        self.N = np.asscalar( np.load(fp) )
        act_text = str( np.load(fp) )
        self.h = np.array( np.load(fp) )
        self.z = np.array( np.load(fp) )
        self.mask = np.array( np.load(fp) )
...
class Network():
    ...
    def TopGradient(self, targets):
        ...
        return self.cost_p(self.lyr[-1].h, targets) * self.lyr[-1].sigma_p(self.lyr[-1].h)
    ...
    def FeedForward(self, x):
        ...
        self.lyr[0].h = deepcopy(x) # Set input layer
